...
fn to_uppercase_high() {
    macro_rules! to_uppercase_high_batch_test {
        ( $value:expr, $exp:expr ) => {
            batch_test!($value, to_uppercase_high, $exp);
        }
    }
    ...
    to_uppercase_high_batch_test!(i64::max_value(), "玖佰貳拾貳兆參仟參佰柒拾貳萬零參佰陸拾捌億伍仟肆佰柒拾柒萬伍仟捌佰零柒");
    to_uppercase_high_batch_test!(i64::min_value(), "負玖佰貳拾貳兆參仟參佰柒拾貳萬零參佰陸拾捌億伍仟肆佰柒拾柒萬伍仟捌佰零捌");
    to_uppercase_high_batch_test!(u64::max_value(), "壹仟捌佰肆拾肆兆陸仟柒佰肆拾肆萬零柒佰參拾柒億零玖佰伍拾伍萬壹仟陸佰壹拾伍");
...
fn to_lowercase_high() {
    macro_rules! to_lowercase_high_batch_test {
        ( $value:expr, $exp:expr ) => {
            batch_test!($value, to_lowercase_high, $exp);
        }
    }
    ...
    to_lowercase_high_batch_test!(i64::max_value(), "九百二十二兆三千三百七十二萬零三百六十八億五千四百七十七萬五千八百零七");
    to_lowercase_high_batch_test!(i64::min_value(), "負九百二十二兆三千三百七十二萬零三百六十八億五千四百七十七萬五千八百零八");
    to_lowercase_high_batch_test!(u64::max_value(), "一千八百四十四兆六千七百四十四萬零七百三十七億零九百五十五萬一千六百一十五");
